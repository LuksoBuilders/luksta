...
contract VestingWalletUpgradeable is Initializable, ContextUpgradeable {
    ...
    event LSP7Released(address indexed token, uint256 amount);

    uint256 private _released;
    mapping(address => uint256) private _lsp7released;
    ...
    function release(address token) public virtual {
        ...
        _lsp7released[token] += amount;
        emit LSP7Released(token, amount);
    ...
    function vestedAmount(
        ...
        return
            _vestingSchedule(
                ILSP7DigitalAsset(token).balanceOf(address(this)) +
                    released(token),
                timestamp
            );
    }
